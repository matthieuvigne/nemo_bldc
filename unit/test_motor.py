...
def test_MyActuator():
    ...
    assert m.compute_max_speed_no_deflux(m.kt_q_art *m.iq_nominal) > m.w_max_at_max_torque
...
def test_reduction():
    ...
    assert a.compute_max_speed_no_deflux(0.1) == pytest.approx(m.compute_max_speed_no_deflux(0.1) / rho, 0.05)
